...
from prepare_data import run as _run, Spot
...
def run(
    ...
    if zoom_factor is not None:
        image = zoom(image, (zoom_factor, zoom_factor, 1.), order=0)
    ...
    with h5py.File(counts_file, 'r') as data:
        ...
        spots = (
            pd.read_csv(spots_file, index_col=0, header=None)
            [[4, 5]]
            .rename(columns={4: 'y', 5: 'x'})
        )
        spots = spots.loc[data['matrix']['barcodes'][()].astype(str)]
        spots = spots[mask]
        spots = list(spots.apply(
            lambda x: Spot(x=x['x'] * SCALE, y=x['y'] * SCALE, r=R * SCALE),
            1,
        ))
