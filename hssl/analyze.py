...
def visualize_greyscale(im, mask=None):
    ...
    return (
        (1 - ((im - im.min()) / (im.max() - im.min()) + 0.1) / 1.1 * mask)
        * 255
    ).astype(np.uint8)
...
def analyze(
    ...
    if sample.data is not None:
        data = t.as_tensor(sample.data.values).to(device).float()
    else:
        data = None
    ...
    for img, prefix in [
        (
            ((img_distr.sample().clamp(-1, 1) + 1) / 2)[0]
            .permute(1, 2, 0)
            .detach()
            .cpu()
            .numpy(),
        ...
        (
            ((img_distr.mean.clamp(-1, 1) + 1) / 2)[0]
            .permute(1, 2, 0)
            .detach()
            .cpu()
            .numpy(),
    ...
    for name, data in (("relative", mix), ("absolute", factors)):
        os.makedirs(os.path.join(output_prefix, "factors", name))
        for i, p in enumerate(
            (data[0, f].detach().cpu().numpy() for f in order_factors(std)), 1
        ):
            imwrite(
                os.path.join(
...
def analyze_gene_profiles(
    ...
    data.to_csv(os.path.join(output_prefix, "profiles.csv"), index=False)
        ...
        if genes != []:
            x = x.loc[
                reduce(
                    ...
                    (
                        x.gene.str.contains(
                            g, regex=regex, flags=re.IGNORECASE
                        )
                        for g in genes
                    ),
                )
...
def analyze_genes(
    ...
    idxs, ns = zip(
        *[
            ...
            for xs in [
                [
                    idx
                    for idx, g in enumerate(std.genes)
    ...
    for idx in set(it.chain(*idxs)):
        ...
        imwrite(
            ...
            visualize_greyscale(
                clip(gene_map, 0.01), label_mask.cpu().numpy()
            ),
        )
        imwrite(
            os.path.join(relp, f"{std.genes[idx]}.png"),
            visualize_greyscale(
                clip(gene_map / total, 0.01), label_mask.cpu().numpy()
            ),
        )
...
def impute_counts(
    ...
    label, region = (
        t.as_tensor(to_array(x)).permute(2, 0, 1).long()
    ...
    region = t.tensor(
        np.searchsorted(regions, region), device=device, dtype=t.long
    ).unsqueeze(0)
    ...
    def _sample():
        ...
        d = std.resample()(
            integrated_loadings[2:], t.as_tensor(sample.effects).to(device)
        )

        return (d.mean.detach().cpu(), d.sample().detach().cpu())

    means, samples = (
        t.stack(xs) for xs in zip(*(_sample() for _ in tqdm(range(samples))))
    )
...
def dge(
    ...
    def _replace_regions(region, label):
        region[(label == 1) | ((region != 0) & (region != 255))] = 1
        region[region == 255] = 2
    ...
    def _sample_one(sample, region):
        ...
        region, label = [
            t.as_tensor(x).to(device).permute(2, 0, 1).long()
            for x in (region, label)
        ]
        image = (
            t.as_tensor(image).to(device).permute(2, 0, 1).float()[None, ...]
            / 255
            * 2
            - 1
        )
        ...
        integrated_loadings = integrate_loadings(
            loadings, (t.as_tensor(region).to(device).long().unsqueeze(0)), 2
        )[[0, 2]]

        return std(
            integrated_loadings,
            (t.tensor(sample.effects).to(integrated_loadings).expand(2, -1)),
        ).mean
    ...
    def _sample_all():
        ...
        a, b = (
            x / x.sum()
            for x in t.stack(
                [*it.starmap(_sample_one, zip(samples, regions))]
            ).sum(0)
        )
        return (t.log(a) - t.log(b)).cpu().detach()

    lfcs = t.stack(
        [_sample_all() for _ in tqdm(range(trials), dynamic_ncols=True)]
    )

    low, high = t.stack(
        [
            xs.sort()[0][
                [int(len(xs) * 0.01), len(xs) - 1 - int(len(xs) * 0.01)]
            ]
    ...
    df = pd.DataFrame(
        dict(
            ...
            lfc_high=high.numpy(),
            pnorm=(
                t.distributions.Normal(lfcs.mean(0).abs(), lfcs.std(0)).cdf(0)
            ),
        )
    )
    (
        df.sort_values("lfc_mean").to_csv(
            os.path.join(output, "dge.csv.gz"), index=False
        )
    )

    df["neg_log_cv"] = (t.log(lfcs.mean(0).abs()) - t.log(lfcs.std(0))).numpy()
    plot = (
        pn.ggplot(df)
        + pn.aes("lfc_mean", "neg_log_cv", xmin="lfc_low", xmax="lfc_high")
    ...
    for sample, region in zip(samples, regions):
        ...
        annotation = (
            ...
            / 2
            * np.concatenate(
                [(region == 0), np.zeros((*region.shape[:2], 2))], 2
            )
            + 1
            / 2
            * np.concatenate(
                [np.zeros((*region.shape[:2], 2)), (region == 2)], 2
            )
        )
        imwrite(
            os.path.join(output, f"{sample.name}_annotation.png"), annotation
        )
